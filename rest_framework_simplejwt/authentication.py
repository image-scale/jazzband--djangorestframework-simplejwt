...
AUTH_HEADER_TYPE_BYTES = {h.encode() for h in AUTH_HEADER_TYPES}


def default_user_authentication_rule(user):
    """
    Default authentication rule that checks if the user is active.
    """
...
class JWTAuthentication(authentication.BaseAuthentication):
    ...
    def get_user(self, validated_token):
        ...
        if api_settings.CHECK_REVOKE_TOKEN:
            revoke_claim = validated_token.get(api_settings.REVOKE_TOKEN_CLAIM)
            if revoke_claim is None or revoke_claim != get_md5_hash_password(user.password):
                raise AuthenticationFailed(
